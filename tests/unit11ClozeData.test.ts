...
describe('Unit 11 cloze content quality', () => {
  it('contains 123 entries and 738 complete cloze records', () => {
    expect(unit11.entries).toHaveLength(123);
    expect(unit11.entries.flatMap(allCloze)).toHaveLength(738);
    for (const entry of unit11.entries) {
  ...
  it('fills every sentence with the canonical Unit 11 word exactly', () => {
    for (const entry of unit11.entries) {
  ...
  it.each(['easy', 'medium', 'hard'] as const)(
    'builds resolving %s sessions for every Unit 11 entry',
    (difficulty) => {
      const entries = unit11.entries.map((entry) => getEntry(entry.entryId)!);
      ...
      for (const question of questions) {
        ...
        expect(new Set(question.options!.map((option) => normalizeLabel(option.label))).size).toBe(4);
        expect(question.options!.filter((option) => option.entryId === question.answer)).toHaveLength(1);
      }
    },
  );
});
